...
/**
 * This is a copy of the PoissonSampler modified so the internal Gaussian sampler
 * can be reset to ensure all tests generate the same random samples.
 */
public class FixedCachePoissonSampler
extends SamplerBase
implements DiscreteSampler,ResettingPoissonSampler {
     ...
     */
    public FixedCachePoissonSampler(UniformRandomProvider rng,
                          double mean) {
        super(rng);
        if (mean <= 0) {
